...
export class ColumnSortPanel extends HTMLElement {
    constructor() {
        ...
        this.dragHandlerMouseDown = (event) => {
            this.dragState.listener(true);
            this.dragState.userSelect('disable');
            this.dragState.activeHandleElement = event.currentTarget;
            ...
            if (this.dragState.activeListElement) { // Having element
                ...
                if (this.dragState.dragStart) {
                    ...
                    if (rect) {
                        ...
                        const targetEl = event.target.closest('[data-element="list-item"]:not(.heading-drag-place-holder)');
                        if (targetEl &&
            ...
            if (isAfterSuccessfullDrag) {
                if (this.el.listContainer && this.el.listContainer.children) {
                    for (let elx_index = 0; elx_index < this.el.listContainer.children.length; elx_index++) {
                        this.ColumnMap[this.el.listContainer.children.item(elx_index).dataset.sortUiId].order = elx_index;
                    }
                }
            }
        };
    ...
    panelInit() {
        this.meta.visibleColumns.sort((a, b) => a.order - b.order);
        this.ColumnMap = {};
        this.meta = getMeta();
    ...
    renderUpdate() {
        ...
        if (this.el.listContainer) {
            ...
            if (this.meta.visibleColumns.length > 0) {
                this.el.listContainer.innerHTML = this.meta.visibleColumns.map((col) => {
                    return ( /* html */`
    ...
    dynamicContentListener(action) {
        if (action === 'bind') {
            this.querySelectorAll('[data-element="list-container"] [data-element="list-item"] [data-element="list-sort-handle"]')
    ...
    generatePlaceHolderAndGhost(el) {
        if (this.dragState.placeHolder) {
            this.dragState.placeHolder.remove();
        }
        if (this.dragState.ghost) {
        ...
        this.dragState.placeHolder = el.cloneNode();
        ...
        this.dragState.ghost = document.createElement('div');
        this.dragState.ghost.innerHTML = el.innerHTML;
        let targetCheck = this.dragState.ghost.querySelector('[data-element="list-toggle"]');
        let source = el.querySelector('[data-element="list-toggle"]');
...
//# sourceMappingURL=column-sort-panel.js.map