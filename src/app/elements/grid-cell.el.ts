import { IGridColumn, ICellPack, IGridRow, ICellData } from './../../../types/grid.d';
...
export class GridCell extends HTMLElement {
	...
	private _ROW: IGridRow;
	private _CELLPACK: ICellPack;

	private cellRef: ICellData;
	private cellScope: 'GLOBAL' | 'LOCAL'; //if it is local push it to global
	...
	ready() {
		...
		if (this.meta.activeFocus) {
			if (
				this._COLUMN.column_unique_id === this.meta.activeFocus.column_unique_id &&
	...
	set cellData(_cellData: ICellPack) {
